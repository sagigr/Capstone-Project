...
Now I want to sort the n-grams to get the top 10 used n-grams in the each data set:

```{r sortgrams, cache=TRUE, warning=FALSE}
...
Let's build the plots of top 10 used n-grams for each data set:

```{r plots, cache=TRUE, warning=FALSE}
library(ggplot2)
...
I plan to make even more cleaning and combining data from all three files, and making a representive sampling, to build a model and an algorithm to predict the next word, implementing it through Shiny App.
